...
fn kmeans_plus_plus(k: u8, mut rng: &mut impl Rng, data: &[Oklab], centroids: &mut Vec<Oklab>, weights: &mut [f32]) {
	use rand::{
	...
	for i in 1..usize::from(k) {
		...
		for (weight, &color) in weights.iter_mut().zip(data) {
			*weight = f32::min(*weight, squared_distance(color, centroid));
		}
...
fn update_distances(centroids: &[Oklab], distances: &mut [(u8, f32)]) {
	let k = centroids.len();
	for i in 0..k {
		let ci = centroids[i];
		...
		for j in (i + 1)..k {
			let cj = centroids[j];
			let dist = squared_distance(ci, cj) / 4.0;
...
fn update_assignments(
	...
	for (&color, &n, center) in soa_zip!(data, [color, count], points.assignment.iter_mut()) {
		...
		for &(other_center, half_dist) in &distances[(ci * k + 1)..((ci + 1) * k)] {
			...
			let other_dist = squared_distance(color, centers.centroid[usize::from(other_center)]);
...
fn update_centroids(rng: &mut impl Rng, centers: &mut CenterDataVec) {
	for (centroid, delta, &n, sum) in soa_zip!(centers, [mut centroid, mut delta, count, sum]) {
		...
		*delta = squared_distance(*centroid, new_centroid).sqrt();
...
fn kmeans(
	...
	kmeans_plus_plus(k, &mut rng, &data.color, &mut centers.centroid, &mut points.weight);
	...
	loop {
		update_distances(&centers.centroid, distances);
		update_assignments(data, centers, distances, points);
		update_centroids(&mut rng, centers);
	...
	let variance = soa_zip!(data, [color, count], &points.assignment)
		.map(|(&color, &n, &center)| {
			f64::from(n) * f64::from(squared_distance(color, centers.centroid[usize::from(center)]))
		})
		.sum();
...
pub fn run_trials(data: &PixelDataVec, trials: u32, k: u8, max_iter: u32, convergence: f32, seed: u64) -> KmeansResult {
	// data.len() <= u32::MAX because of `get_thumbnail` and `process_pixels`
	#[allow(clippy::cast_possible_truncation)]
	let mut state = KmeansState::new(k, data.len() as u32);

	(0..trials)
		.map(|i| kmeans(data, &mut state, k, max_iter, convergence, seed + u64::from(i)))
